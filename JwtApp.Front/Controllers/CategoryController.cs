    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Remove(int id)
        {
            ...
            if (token != null)
            {
                var client = _httpClientFactory.CreateClient();
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                await client.DeleteAsync($"http://localhost:5044/api/categories/{id}");
            }
        ...
        public async Task<IActionResult> Create(CreateCategoryModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.PostAsync("http://localhost:5044/api/categories", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            if (token != null)
            {
                ...
                var response = await client.GetAsync($"http://localhost:5044/api/categories/{id}");
                if (response.IsSuccessStatusCode)
                {
                    var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Update(CategoryListModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.PutAsync("http://localhost:5044/api/categories", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
